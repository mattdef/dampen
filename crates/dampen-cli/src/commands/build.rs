...
pub struct BuildArgs {
    ...
    /// Production build (use cargo build --release)
    #[arg(long)]
    prod: bool,
...
pub fn execute(args: &BuildArgs) -> Result<(), String> {
    if args.prod || args.release {
        return execute_production_build(args);
    }
    ...
    let dampen_files = find_dampen_files(&input_dir);
    if dampen_files.is_empty() {
        return Err(format!("No .dampen files found in '{}'", args.input));
    }

    if args.verbose {
        eprintln!("Found {} .dampen files", dampen_files.len());
    }

    // Generate code from all files
    let generated_code =
        generate_code_from_files(&dampen_files, &args.model, &args.message, args.verbose)?;
...
fn find_dampen_files(dir: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            } else if path.extension().is_some_and(|ext| ext == "dampen") {
                files.push(path);
            }
        }
    }
...
fn generate_code_from_files(
    ...
    code.push_str("//! DO NOT EDIT - Generated by `dampen build`\n\n");
    code.push_str("use dampen_core::{parse, generate_application, HandlerSignature};\n\n");
