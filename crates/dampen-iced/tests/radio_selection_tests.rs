...
fn test_radio_selection_changes() {
    ...
    let _element_a = DampenWidgetBuilder::new(&document, &model_a, Some(&registry)).build();
    ...
    let _element_b = DampenWidgetBuilder::new(&document, &model_b, Some(&registry)).build();
...
fn test_radio_without_handler() {
    ...
    let _element = DampenWidgetBuilder::new(&document, &model, None).build();
