...
fn test_radio_default_preserves_across_rebuilds() {
    ...
    let _element1 = GravityWidgetBuilder::new(&document, &model, Some(&registry)).build();
    let _element2 = GravityWidgetBuilder::new(&document, &model, Some(&registry)).build();
    let _element3 = GravityWidgetBuilder::new(&document, &model, Some(&registry)).build();
