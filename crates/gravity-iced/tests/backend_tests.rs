...
fn test_stack_widget() {
    let backend = create_test_backend();
    let children = vec![
        backend.text("Layer 1"),
        backend.text("Layer 2"),
    ];
...
fn test_nested_layouts() {
    ...
    let row1 = backend.row(vec![
        backend.text("Left"),
        backend.text("Right"),
    ]);
    
    let row2 = backend.row(vec![
        backend.text("A"),
        backend.text("B"),
    ]);
...
fn test_message_handler_invocation() {
    ...
    let backend = IcedBackend::new(|name, _value| {
