...
use gravity_core::{
    parse, AttributeValue, EventBinding, EventKind, WidgetKind, WidgetNode,
};
...
fn test_render_checkbox() {
    let backend = create_backend();
    let xml = r#"<checkbox label="Enable feature" checked="{is_enabled}" on_toggle="toggle_feature" />"#;
    let doc = parse(xml).unwrap();
...
fn test_render_pick_list() {
    let backend = create_backend();
    let xml = r#"<pick_list options="Red,Green,Blue" selected="{color}" on_select="select_color" />"#;
    let doc = parse(xml).unwrap();
