...
static GRAVITY_DOCUMENT: LazyLock<gravity_core::GravityDocument> = LazyLock::new(|| {
    let xml = r#"
            <gravity>
...
fn test_lazy_lock_initialization() {
    let doc = &GRAVITY_DOCUMENT;
    assert_eq!(doc.root.kind, gravity_core::ir::WidgetKind::Column);
}

#[test]
fn test_multiple_views_pattern() {
    static VIEW1: LazyLock<gravity_core::GravityDocument> = LazyLock::new(|| {
        let xml = r#"<gravity><column><text value="View 1" /></column></gravity>"#;
        parse(xml).expect("Failed to parse View 1")
    });

    static VIEW2: LazyLock<gravity_core::GravityDocument> = LazyLock::new(|| {
