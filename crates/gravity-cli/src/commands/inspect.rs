...
fn print_ir_tree(doc: &gravity_core::GravityDocument, indent: usize) {
...
fn print_widget_node(node: &gravity_core::WidgetNode, indent: usize) {
    let prefix = "  ".repeat(indent);
...
fn print_attribute_value(value: &gravity_core::AttributeValue) {
    match value {
        gravity_core::AttributeValue::Static(s) => {
            print!("Static({:?})", s);
        }
        gravity_core::AttributeValue::Binding(expr) => {
            print!("Binding(");
            print_expr(&expr.expr);
            print!(")");
        }
        gravity_core::AttributeValue::Interpolated(parts) => {
            print!("Interpolated([");
            for (i, part) in parts.iter().enumerate() {
                ...
                match part {
                    gravity_core::InterpolatedPart::Literal(s) => {
                        print!("Literal({:?})", s);
                    }
                    gravity_core::InterpolatedPart::Binding(expr) => {
                        print!("Binding(");
                        print_expr(&expr.expr);
                        print!(")");
                    }
...
fn print_expr(expr: &gravity_core::Expr) {
    match expr {
        gravity_core::Expr::FieldAccess(fa) => {
            print!("FieldAccess({})", fa.path.join("."));
        }
        gravity_core::Expr::MethodCall(mc) => {
            print!("MethodCall(");
        ...
        gravity_core::Expr::UnaryOp(uo) => {
            print!("UnaryOp({:?} ", uo.op);
            print_expr(&uo.operand);
            print!(")");
        }
        gravity_core::Expr::Conditional(ce) => {
            print!("Conditional(");
        ...
        gravity_core::Expr::Literal(lit) => match lit {
            gravity_core::LiteralExpr::String(s) => print!("Literal({:?})", s),
            gravity_core::LiteralExpr::Integer(i) => print!("Literal({})", i),
            gravity_core::LiteralExpr::Float(f) => print!("Literal({})", f),
            gravity_core::LiteralExpr::Bool(b) => print!("Literal({})", b),
        },
    }
}
