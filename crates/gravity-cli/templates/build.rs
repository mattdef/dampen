//! Build script for Gravity projects
//!
//! This build script scans the ui/ directory for .gravity files and generates
...
fn main() {
    ...
    // Scan for .gravity files
    let gravity_files = find_gravity_files(&ui_dir);
    
    if gravity_files.is_empty() {
        println!("cargo:warning=No .gravity files found in ui/ directory");
        return;
    }
    
    // Generate code
    let generated_code = generate_from_files(&gravity_files);
    ...
    println!("cargo:rustc-env=GRAVITY_GENERATED={}", output_file.display());
    
    // Tell cargo to rerun if any .gravity files change
...
/// Find all .gravity files in a directory recursively
fn find_gravity_files(dir: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            } else if path.extension().map_or(false, |ext| ext == "gravity") {
                files.push(path);
            }
        }
    }
...
fn generate_from_files(files: &[PathBuf]) -> String {
    ...
    code.push_str("use gravity_core::parse;\n");
    code.push_str("use gravity_core::codegen::generate_application;\n");
    code.push_str("use gravity_core::HandlerSignature;\n\n");
