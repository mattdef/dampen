//! Tests for <dampen> document parsing

use dampen_core::parse;

#[test]
fn test_parse_dampen_with_themes() {
    ...
    assert_eq!(
        doc.global_theme.unwrap(),
        "custom",
        "Global theme should be 'custom'"
    );
    assert!(
        doc.themes.contains_key("custom"),
        "Should have 'custom' theme"
    );
    assert_eq!(
        doc.root.kind,
        dampen_core::WidgetKind::Column,
        "Root should be column"
    );
}

#[test]
fn test_parse_dampen_without_themes() {
    let xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<dampen>\n    <column padding=\"20\">\n        <text value=\"Hello\" />\n    </column>\n</dampen>";
...
fn test_parse_backward_compatibility() {
    // Old format without <dampen> should still work
...
#[test]
fn test_parse_dampen_with_style_classes() {
    ...
    assert!(
        doc.style_classes.contains_key("highlight"),
        "Should have 'highlight' class"
    );
}

#[test]
fn test_parse_dampen_multiple_widgets_error() {
