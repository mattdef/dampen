...
fn test_multiple_file_events() {
    ...
    fs::write(&file1, "<dampen><text value=\"File 1\" /></dampen>")
        .expect("Failed to create file1");

    thread::sleep(Duration::from_millis(30));

    fs::write(&file2, "<dampen><text value=\"File 2\" /></dampen>")
...
fn test_error_recovery_simulation() {
    ...
    fs::write(&invalid_file, "<dampen><broken").expect("Failed to create invalid file");

    thread::sleep(Duration::from_millis(80));

    // Create valid file
    let valid_file = temp_dir.path().join("valid.dampen");
    fs::write(&valid_file, "<dampen><text value=\"Valid\" /></dampen>")
