...
use crate::{GravityDocument, WidgetKind, AttributeValue, InterpolatedPart};
use quote::quote;
use proc_macro2::TokenStream;

/// Generate the view function from a Gravity document
...
fn generate_text(node: &crate::WidgetNode, model_ident: &syn::Ident) -> Result<TokenStream, super::CodegenError> {
    let value_attr = node.attributes.get("value")
        .ok_or_else(|| super::CodegenError::InvalidWidget("text requires value attribute".to_string()))?;
...
fn generate_button(
    ...
    let label_attr = node.attributes.get("label")
        .ok_or_else(|| super::CodegenError::InvalidWidget("button requires label attribute".to_string()))?;
...
fn generate_container(
    ...
    let children: Vec<TokenStream> = node.children
        .iter()
    ...
    let spacing = node.attributes.get("spacing")
        .and_then(|attr| {
 ...
 -> TokenStream {
    match attr {
        ...
        AttributeValue::Binding(expr) => {
            generate_binding_expr(expr, model_ident)
        }
        AttributeValue::Interpolated(parts) => {
            let parts_tokens: Vec<TokenStream> = parts.iter()
                .map(|part| match part {
