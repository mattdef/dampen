use gravity_core::{HandlerRegistry, ParseError, Span};
use gravity_runtime::{
    interpreter::HotReloadInterpreter, overlay::ErrorOverlay, state::RuntimeState,
    watcher::FileWatcher,
};
use std::fs;
use std::thread;
...
fn main() {
    ...
    match result {
        gravity_runtime::interpreter::ReloadResult::Success {
            latency_ms,
            state_restored,
        } => {
